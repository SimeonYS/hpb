...
class HpbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('hpb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `hpb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM hpb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO hpb (date, title, link, content)"
